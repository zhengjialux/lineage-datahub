...
import type { VizNode, EntitySelectParams, EntityAndType, UpdatedLineages } from './types';
...
export default function LineageEntityNode({
    ...
    function fetchEntityLineage() {
        if (node.data.urn) {
            if (isCenterNode) {
                ...
                getAsyncEntityLineage(node.data.urn)
    ...
    const properties = JSON.stringify(entityStructuredProps?.properties) !== "{}" ? entityStructuredProps?.properties : []
    const hasAssetBadge = properties?.find(filterForAssetBadge);
