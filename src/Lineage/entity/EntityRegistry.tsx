...
import type { Entity as EntityInterface, Exact, SearchResult } from '../types.generated';
import { EntityType } from '../types.generated';
import type { FetchedEntity } from '../lineage/types';
import { SearchResultProvider } from '../search/context/SearchResultContext';
import { EntityCapabilityType, IconStyleType, PreviewType } from './Entity';
import type { Entity } from './Entity';
import { GLOSSARY_ENTITY_TYPES } from './shared/constants';
import type { EntitySidebarSection, GenericEntityProperties } from './shared/types';
...
export default class EntityRegistry {
    ...
    getLineageVizConfig<T>(type: EntityType, data: T): FetchedEntity | undefined {
        ...
        const downstreamRelationships = JSON.stringify(genericEntityProperties?.downstream?.relationships) !== '{}' ? genericEntityProperties?.downstream?.relationships : []
        const upstreamRelationships = JSON.stringify(genericEntityProperties?.upstream?.relationships) !== "{}" ? genericEntityProperties?.upstream?.relationships : []

        return (
            ({
                ...entity.getLineageVizConfig?.(data),
                downstreamChildren: downstreamRelationships
                ...
                downstreamRelationships: downstreamRelationships?.filter(
                    (relationship) => relationship.entity,
                ),
                numDownstreamChildren:
                    (genericEntityProperties?.downstream?.total || 0) -
                    (genericEntityProperties?.downstream?.filtered || 0),
                upstreamChildren: upstreamRelationships
                    ?.filter((relationship) => relationship.entity)
                    ?.map((relationship) => ({
                        entity: relationship.entity as EntityInterface,
                        type: (relationship.entity as EntityInterface).type,
                    })),
                upstreamRelationships: upstreamRelationships?.filter(
                    (relationship) => relationship.entity,
                ),
                numUpstreamChildren:
