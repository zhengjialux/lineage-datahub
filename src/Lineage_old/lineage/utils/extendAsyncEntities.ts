import type { SchemaFieldRef } from '../../types.generated';
import { EntityType } from '../../types.generated';
import EntityRegistry from '../../entity/EntityRegistry';
import type { EntityAndType, FetchedEntities, FetchedEntity } from '../types';
import {
    decodeSchemaField,
...
export function extendColumnLineage(
            ...
            fineGrainedLineage.upstreams?.forEach((upstream) => {
                ...
                fineGrainedLineage.downstreams?.forEach((downstream) => {
                    ...
                    Array.from(fetchedEntities.keys()).forEach((urn) => {
                        const searchResults = JSON.stringify(fetchedEntities.get(urn)?.siblingsSearch?.searchResults)!== '{}'? fetchedEntities.get(urn)?.siblingsSearch?.searchResults : []
    ...
    const searchResults = JSON.stringify(lineageVizConfig.siblingsSearch?.searchResults)!== '{}'? lineageVizConfig.siblingsSearch?.searchResults : []
    searchResults?.forEach((sibling) => {
