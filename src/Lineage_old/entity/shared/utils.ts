...
import type { Entity, EntityRelationshipsResult, DataProduct, PropertyValue } from '../../types.generated';
...
export function getFineGrainedLineageWithSiblings(
    ...
    const entityDataFineGrainedLineages = JSON.stringify(entityData?.fineGrainedLineages) !== '{}' ? entityData?.fineGrainedLineages : [];
    const fineGrainedLineages = [
        ...(entityDataFineGrainedLineages || entityData?.inputOutput?.fineGrainedLineages || []),
    ];
    const searchResults = JSON.stringify(entityData?.siblingsSearch?.searchResults) !== '{}' ? entityData?.siblingsSearch?.searchResults : []
    searchResults?.forEach((sibling) => {
        if (sibling.entity) {
